...
class WakeWordManager {
  ...
  async runInference(chunk, isSpeechConsideredActive) {
    ...
    const new_mel_data = melspecResults[this.melspecModel.outputNames[0]].data;
    ...
    while (this.mel_buffer.length >= 76) {
      ...
      for (const name in this.models) {
        ...
        if (score > this.detectionThreshold && isSpeechConsideredActive && !this.isDetectionCoolingDown) {
  ...
  /**
     * Update tuning settings for wake word detection
     */
  updateTuningSettings(settings) {
    console.log('Updating wake word tuning settings:', settings);

    // Update detection threshold
    if (settings.detectionThreshold !== undefined) {
      this.detectionThreshold = settings.detectionThreshold;
      console.log(`Detection threshold updated to: ${this.detectionThreshold}`);
    }
    ...
    if (settings.vadSensitivity !== undefined) {
      ...
      console.log(`VAD sensitivity updated to: ${this.vadThreshold}`);
    }
